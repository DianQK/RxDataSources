...
struct Randomizer {
    ...
    func randomize() -> Randomizer {
        ...
        let itemCount = countTotalItemsInSections(sections: sections)
        ...
        for section in self.unusedSections {
            ...
            if insertSections {
                sections.insert(NumberSection(header: section, numbers: [], updated: NSDate()), at: index)
            }
            else {
        ...
        assert(countTotalItemsInSections(sections: sections) + nextUnusedItems.count == startItemCount)
