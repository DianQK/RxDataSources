...
public func differencesForSectionedView<S: AnimatableSectionModelType>(
    ...
    var sectionCommands = try CommandGenerator<S>.generatorForInitialSections(initialSections: initialSections, finalSections: finalSections)
...
struct CommandGenerator<S: AnimatableSectionModelType> {
    ...
    static func generatorForInitialSections(
        ...
        let (initialSectionData, finalSectionData) = try calculateSectionMovementsForInitialSections(initialSections: initialSections, finalSections: finalSections)
        let (initialItemData, finalItemData) = try calculateItemMovementsForInitialSections(initialSections: initialSections,
    ...
    static func calculateItemMovementsForInitialSections(initialSections: [S], finalSections: [S],
        ...
        let initialItemIndexes = try indexSectionItems(sections: initialSections)
        ...
        for i in 0 ..< finalSections.count {
            ...
            for j in 0 ..< finalSections[i].items.count {
                ...
                // original section can't be inserted
                    ...
                    try rxPrecondition(condition: false, "New section in initial sections, that is wrong")
                }

                let initialSectionEvent = initialSectionData[originalIndex.sectionIndex].event
                try rxPrecondition(condition: initialSectionEvent == .moved || initialSectionEvent == .movedAutomatically, "Section not moved")
    ...
    static func calculateSectionMovementsForInitialSections<S: AnimatableSectionModelType>(initialSections: [S], finalSections: [S]) throws -> ([SectionAssociatedData], [SectionAssociatedData]) {

        let initialSectionIndexes = try indexSections(sections: initialSections)
    ...
    mutating func generateDeleteSections() throws -> [Changeset<S>] {
        ...
        for (i, initialSection) in initialSections.enumerated() {
            ...
            for j in 0 ..< initialSection.items.count {
                ...
                default:
                    try rxPrecondition(condition: false, "Unhandled case")
                }
            }
    ...
    func generateInsertAndMoveSections() throws -> [Changeset<S>] {
        ...
        for i in 0 ..< initialSections.count {
            ...
            case .movedAutomatically:
                break
            default:
                try rxPrecondition(condition: false, "Unhandled case in initial sections")
            }
        }
            ...
            if event == .inserted {
                ...
                for (j, _) in originalSection.items.enumerated() {
                    ...
                    guard let finalIndex = initialData.moveIndex else {
                        try rxPrecondition(condition: false, "Item was moved, but no final location.")
                        continue
                    }
                ...
                try rxPrecondition(condition: false, "This is weird, this shouldn't happen")
                return s
            }
        }
    ...
    mutating func generateNewAndMovedItems() throws -> [Changeset<S>] {
        ...
        for i in 0 ..< finalSections.count {
            ...
            for j in 0 ..< finalSection.items.count {
                ...
                try rxPrecondition(condition: currentItemEvent != .untouched, "Current event is not untouched")
